...
public class Region  {
	...
	public boolean matchesIBeacon(IBeacon iBeacon) {
		if (proximityUuid != null && !iBeacon.getProximityUuid().equals(proximityUuid)) {
			if (IBeaconManager.debug) Log.d(TAG, "unmatching proxmityUuids: "+iBeacon.getProximityUuid()+" != "+proximityUuid);
			return false;
		}
		if (major != null && iBeacon.getMajor() != major) {
			if (IBeaconManager.debug) Log.d(TAG, "unmatching major: "+iBeacon.getMajor()+" != "+major);
			return false;
		}
		if (minor != null && iBeacon.getMinor() != minor) {
			if (IBeaconManager.debug) Log.d(TAG, "unmatching minor: "+iBeacon.getMajor()+" != "+minor);
			return false;
		}
		return true;
	}
